...
import Test.Hspec
import Test.Hspec.QuickCheck qualified as Hspec.QC
...
spec = do
  describe "Implementations are equivalent" $ do
    Hspec.QC.prop
      "ByTheBook and Megaparsec implementations give the same results"
...
parserImplementationsAreEquivalent =
 ...
 where
  ...
  genToken = do
    tkType <-
      QC.arbitraryBoundedEnum @Token.TokenType
    ...
    (tkLexeme, tkLit) <- case tkType of
      ...
      _ -> pure ("", Nothing)
    Token tkLexeme tkType tkLit <$> QC.getSize
